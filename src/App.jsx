...
const CATEGORY_RULES = {
  "Food & Dining":        ["swiggy","zomato","coffee","cafe","restaurant","food","chai","biryani","pizza","burger","tea","lunch","dinner","breakfast","starbucks","mcdonalds","kfc","dominos","snacks","juice","bakery","boojee","blue tokai","third wave","subko","zomato"],
  ...
  "Groceries":            ["blinkit","zepto","bigbasket","dmart","grofers","instamart","vegetables","fruits","milk","grocery","kirana","supermarket","ration","reliance fresh","jiomart","beans","boojee_beans"],
...
export default function ExpenseTracker() {
  ...
  if (loading) return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100vh",background:"#0B1120"}}>
  ...
  const pad = isDesktop ? "24px 40px" : isTablet ? "20px 28px" : "16px 14px";

  const categorySection = (
    <section style={S.section}>
      <h3 style={S.sectionTitle}>By Category</h3>
      <div style={{display:"flex",alignItems:"center",gap:16}}>
        <ResponsiveContainer width={isDesktop ? 200 : 180} height={isDesktop ? 200 : 180}>
          <PieChart>
            <Pie data={catTotals.map(([n,v])=>({name:n,value:v}))} dataKey="value" cx="50%" cy="50%" outerRadius={isDesktop?85:70} innerRadius={isDesktop?52:42}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",gap:8}}>
          {catTotals.map(([n,a])=>(
            <div key={n} style={{display:"flex",alignItems:"center",gap:8,fontSize:isDesktop?13:12}}>
              <div style={{width:8,height:8,borderRadius:"50%",background:CATEGORIES[n]?.color||"#6B7280",flexShrink:0}}/>
              <span style={{flex:1,color:"#CBD5E1"}}>{CATEGORIES[n]?.icon} {n}</span>
              <span style={{color:"#fff",fontWeight:600}}>{fmt(a)}</span>
              <span style={{color:"#475569",fontSize:11,width:32,textAlign:"right"}}>{total?Math.round(a/total*100):0}%</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <section style={S.section}>
      ...
      {Object.entries(filtered.reduce((acc,e)=>{acc[e.merchant]=(acc[e.merchant]||0)+e.amount;return acc;},{}))
        ...
        <div key={n} style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
          <span style={{width:isDesktop?130:90,flexShrink:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontSize:13,color:"#CBD5E1"}}>{n}</span>
          <div style={{flex:1,height:6,background:"#1E293B",borderRadius:3,overflow:"hidden"}}>
            <div style={{height:"100%",background:"#6366F1",borderRadius:3,width:`${Math.round(a/total*100)}%`}}/>
          </div>
          <span style={{fontSize:13,color:"#A5B4FC",fontWeight:600,width:80,textAlign:"right"}}>{fmt(a)}</span>
        </div>
      ))}
    </section>
  );
  ...
  const insightsSection = (
    <section style={S.section}>
      ...
      {insights
        ? <div style={{background:"#0F172A",border:"1px solid #1E293B",borderRadius:10,padding:"16px"}}>{insights.split("\n").filter(l=>l.trim()).map((l,i)=><p key={i} style={{margin:"6px 0",lineHeight:1.7,color:l.match(/^\d\./)?"#A5B4FC":"#CBD5E1",fontSize:isDesktop?14:13}}>{l}</p>)}</div>
        : <div style={{color:"#475569",fontSize:13,fontStyle:"italic",textAlign:"center",padding:"20px 0"}}>Hit "Analyse" for AI insights on your {MONTHS[selMonth]} spending.</div>
      }
    </section>
  );
  ...
  const emptyState = (
    <div style={{textAlign:"center",padding:"80px 20px",display:"flex",flexDirection:"column",alignItems:"center",gap:16}}>
      ...
      <p style={{color:"#475569",fontSize:13}}>Message your Telegram bot, then hit Sync.</p>
  ...
  return (
    ...
    <div style={{background:"#0B1120",minHeight:"100vh",color:"#F1F5F9",fontFamily:"'Inter',system-ui,sans-serif"}}>
      {toast && <div style={{position:"fixed",top:20,left:"50%",transform:"translateX(-50%)",background:"#6366F1",color:"#fff",padding:"10px 24px",borderRadius:24,fontSize:13,fontWeight:500,zIndex:999,whiteSpace:"nowrap",boxShadow:"0 4px 20px rgba(99,102,241,0.4)"}}>{toast}</div>}

      {/* Content container — centred with max-width */}
      <div style={{maxWidth: isDesktop ? 1280 : isTablet ? 900 : 480, margin:"0 auto"}}>

        {/* HEADER */}
        <header style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:pad,borderBottom:"1px solid #1E293B"}}>
          <div>
            <div style={{fontSize:isDesktop?30:22,fontWeight:700,letterSpacing:"-0.5px",color:"#fff"}}>Kharcha</div>
            <div style={{fontSize:11,color:"#475569",marginTop:2}}>{lastSync?`Synced ${lastSync.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}`:"Not synced yet"}</div>
          </div>
        ...
        {syncError && <div style={{background:"#1F0A0A",border:"1px solid #7F1D1D",color:"#FCA5A5",padding:"10px 16px",fontSize:12}}>{syncError}</div>}

        {/* NAV */}
        <nav style={{display:"flex",borderBottom:"1px solid #1E293B",padding:`0 ${isDesktop?"40px":isTablet?"28px":"0"}`}}>
          {["dashboard","transactions","categories"].map(tab=>(
            <button key={tab} style={{flex:1,padding:isDesktop?"14px 8px":"12px 4px",background:"none",border:"none",color:activeTab===tab?"#6366F1":"#64748B",fontSize:isDesktop?14:12,fontWeight:500,cursor:"pointer",borderBottom:activeTab===tab?"2px solid #6366F1":"2px solid transparent"}} onClick={()=>setActiveTab(tab)}>
        ...
        {/* MAIN */}
        <main style={{padding:pad, paddingBottom:60}}>
          ...
          {activeTab==="dashboard" && (
            filtered.length===0 ? emptyState : (
              <div>
                {/* Total card */}
                <div style={{background:"linear-gradient(135deg,#1E1B4B,#1E293B)",border:"1px solid #312E81",borderRadius:16,padding:isDesktop?"32px 40px":"24px 20px",marginBottom:24,textAlign:"center"}}>
                  <div style={{fontSize:11,color:"#818CF8",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8}}>Total spent · {MONTHS[selMonth]} {selYear}</div>
                  <div style={{fontSize:isDesktop?56:38,fontWeight:700,color:"#fff",letterSpacing:"-1px"}}>{fmt(total)}</div>
                  <div style={{fontSize:12,color:"#475569",marginTop:6}}>{filtered.length} transactions</div>
                </div>

                {isDesktop ? (
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
                    <div>{categorySection}{merchantSection}</div>
                    <div>{dailySection}{insightsSection}</div>
                  </div>
                ) : (
                  <div>{categorySection}{dailySection}{merchantSection}{insightsSection}</div>
                )}
              </div>
            )
          )}
          ...
          {activeTab==="transactions" && (
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
                ...
                <span style={{background:"#1E293B",color:"#A5B4FC",borderRadius:8,padding:"5px 12px",fontSize:13,fontWeight:700}}>{fmt(total)}</span>
              </div>
              {filtered.length===0
                ? <div style={{textAlign:"center",padding:"60px 20px",color:"#64748B"}}>No transactions this month.</div>
                : isDesktop ? (
                  <table style={{width:"100%",borderCollapse:"collapse",background:"#111827",borderRadius:14,overflow:"hidden",border:"1px solid #1E293B"}}>
                    <thead>
                      <tr style={{borderBottom:"1px solid #1E293B"}}>
                        {["","Date","Merchant","Category","Amount","Note"].map(h=>(
                          <th key={h} style={{padding:"12px 16px",textAlign:"left",fontSize:11,color:"#475569",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.06em"}}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {[...filtered].sort((a,b)=>new Date(b.date)-new Date(a.date)).map(exp=>(
                        <tr key={exp.id} style={{borderBottom:"1px solid #1E293B"}}>
                          <td style={{padding:"12px 16px",fontSize:20}}>{CATEGORIES[exp.category]?.icon||"📦"}</td>
                          <td style={{padding:"12px 16px",fontSize:13,color:"#64748B"}}>{exp.date}</td>
                          <td style={{padding:"12px 16px",fontSize:14,fontWeight:600,color:"#F1F5F9"}}>{exp.merchant}</td>
                          <td style={{padding:"12px 16px",fontSize:13,color:"#94A3B8"}}>{exp.category}</td>
                          <td style={{padding:"12px 16px",fontSize:14,fontWeight:700,color:"#A5B4FC"}}>{fmt(exp.amount)}</td>
                          <td style={{padding:"12px 16px",fontSize:13,color:"#64748B"}}>{exp.note}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  [...filtered].sort((a,b)=>new Date(b.date)-new Date(a.date)).map(exp=>(
                    <div key={exp.id} style={{display:"flex",alignItems:"flex-start",gap:14,padding:"14px 0",borderBottom:"1px solid #1E293B"}}>
                      <div style={{fontSize:22,width:36,textAlign:"center",flexShrink:0}}>{CATEGORIES[exp.category]?.icon||"📦"}</div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:14,fontWeight:600,color:"#F1F5F9"}}>{exp.merchant}</div>
                        <div style={{fontSize:11,color:"#64748B",marginTop:3}}>{exp.category} · {exp.date}{exp.note?` · ${exp.note}`:""}</div>
                      </div>
                      <div style={{fontSize:15,fontWeight:700,color:"#A5B4FC",flexShrink:0}}>{fmt(exp.amount)}</div>
                    </div>
                  ))
                )
              }
          ...
          {activeTab==="categories" && (
            <div>
              <div style={S.section}>
                ...
                <p style={{color:"#64748B",fontSize:13,marginBottom:20}}>Merchants are matched using these keywords on every sync.</p>
                <div style={isDesktop?{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}:{}}>
                  {Object.entries(CATEGORY_RULES).map(([cat,keys])=>(
                    <div key={cat} style={{marginBottom:16,paddingBottom:16,borderBottom:"1px solid #1E293B"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                        <div style={{width:9,height:9,borderRadius:"50%",background:CATEGORIES[cat]?.color||"#6B7280",flexShrink:0}}/>
                        <span style={{fontSize:isDesktop?14:13,fontWeight:600,color:"#E2E8F0"}}>{CATEGORIES[cat]?.icon} {cat}</span>
                      </div>
                      <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                        {keys.slice(0,8).map(k=><span key={k} style={{background:"#1E293B",color:"#64748B",borderRadius:12,padding:"3px 9px",fontSize:11}}>{k}</span>)}
                        {keys.length>8 && <span style={{color:"#475569",fontSize:11,padding:"3px 4px"}}>+{keys.length-8} more</span>}
                      </div>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{background:"#0C1F3F",border:"1px solid #1D3A6E",borderRadius:14,padding:"18px"}}>
                <div style={{fontSize:14,fontWeight:700,color:"#60A5FA",marginBottom:14}}>📱 Message format</div>
                <div style={isDesktop?{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}:{}}>
                  {[["coffee 200","Food & Dining · ₹200"],["uber 350 airport","Transport · ₹350"],["blinkit 650","Groceries · ₹650"],["netflix 649","Entertainment · ₹649"],["emi 15000","EMI & Loans · ₹15,000"],["whey 2500","Protein & Supplements · ₹2,500"]].map(([msg,res])=>(
                    <div key={msg} style={{display:"flex",gap:10,alignItems:"center",marginBottom:10}}>
                      <code style={{background:"#1E293B",color:"#A5B4FC",borderRadius:6,padding:"4px 10px",fontSize:12,fontFamily:"monospace",whiteSpace:"nowrap"}}>{msg}</code>
